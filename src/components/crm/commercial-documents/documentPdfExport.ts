...
async function waitForDocumentImages(element: HTMLElement): Promise<void> {
  ...
  await Promise.all(images.map((image) => {
    ...
    return new Promise<void>((resolve, reject) => {
      const timeout = window.setTimeout(() => reject(new Error(`Document image did not load: ${image.alt || image.src}`)), 5000);
      image.addEventListener("load", () => { window.clearTimeout(timeout); resolve(); }, { once: true });
      image.addEventListener("error", () => { window.clearTimeout(timeout); reject(new Error(`Document image failed to load: ${image.alt || image.src}`)); }, { once: true });
    });
  }));
}

export async function createDocumentPdfFromElement(element: HTMLElement, fileName: string): Promise<DocumentPdfResult> {
  ...
  const exportId = `document-pdf-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  element.setAttribute("data-document-pdf-export-id", exportId);
  ...
  try {
    canvas = await html2canvas(element, {
      ...
      onclone: (clonedDocument) => {
        const clonedElement = clonedDocument.querySelector<HTMLElement>(`[data-document-pdf-export-id="${exportId}"]`);
        if (!clonedElement) throw new Error("Document PDF clone is unavailable.");
        applyPdfSafeComputedColors(element, clonedElement);
      },
    });
  } finally {
    element.removeAttribute("data-document-pdf-export-id");
  }
...
export function downloadDocumentPdf(result: DocumentPdfResult): void {
