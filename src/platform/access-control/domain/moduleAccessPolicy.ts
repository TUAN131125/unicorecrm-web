...
export const MODULE_ACCESS_POLICIES: Readonly<Record<string, ModuleAccessPolicy>> = {
  ...
  products: policy("products", "quản trị sản phẩm", "Product administration", [CAPABILITIES.PRODUCTS_CREATE, CAPABILITIES.PRODUCTS_UPDATE, CAPABILITIES.PRODUCTS_CONFIGURE]),
  ...
  shipping: policy("shipping", "bộ phận Vận hành", "Operations", [CAPABILITIES.SHIPPING_CREATE, CAPABILITIES.SHIPPING_RETRY, CAPABILITIES.SHIPPING_CANCEL, CAPABILITIES.SHIPPING_SYNC]),
  returns: policy("returns", "bộ phận Vận hành và Chăm sóc khách hàng", "Operations and Customer Success", [CAPABILITIES.RETURNS_CREATE, CAPABILITIES.RETURNS_UPDATE, CAPABILITIES.RETURNS_APPROVE, CAPABILITIES.RETURNS_RESOLVE]),
