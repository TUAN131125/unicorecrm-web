...
assert.equal(manifest.gates.length, 310);
assert.equal(new Set(manifest.gates.map((gate: { id: string }) => gate.id)).size, 310, "Stable gate IDs must be unique.");
assert.equal(new Set(manifest.gates.map((gate: { commandName: string }) => gate.commandName)).size, 310, "Compatibility command names must be unique.");
...
assert.equal(indexed.gatesById.size, 310);
