...
const outboundBehavior = readFileSync(join(root, "src/workspaces/studio/application/connectedOutboundWebhookBehavior.ts"), "utf8");
for (const operationId of ["getIntegrationEventCatalog", "listOutboundWebhookSubscriptions", "createOutboundWebhookSubscription", "updateOutboundWebhookSubscription", "activateOutboundWebhookSubscription", "pauseOutboundWebhookSubscription", "resumeOutboundWebhookSubscription", "archiveOutboundWebhookSubscription", "rotateOutboundWebhookSecret", "listOutboundWebhookDeliveries", "replayOutboundWebhookDelivery"]) {
  assert.ok((outbound + outboundApi + outboundBehavior).includes(operationId), `Connected outbound webhook boundary must use generated ${operationId}`);
}
...
assert.ok(((outbound + outboundBehavior).match(/await (?:load|refresh)\(\)/gu)?.length ?? 0) >= 4, "Connected mutations must refetch authoritative backend state");
